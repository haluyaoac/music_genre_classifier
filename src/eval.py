...
def main():
    ...
    ap.add_argument("--k", type=int, default=0, help="保留：0 表示按 split_fixed 全部切片融合（推荐）")
    args = ap.parse_args()
    ...
    model = build_model(cfg.model_type, cfg.n_classes).to(device).eval()
    ...
    # 数据
    items = load_split_items(cfg.split_json, args.split)
    if len(items) == 0:
        raise RuntimeError(f"No items found for split={args.split} in {cfg.split_json}")
